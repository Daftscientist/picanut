...
logger = logging.getLogger("canopy.webhooks")


async def _resolve_token(token: str, conn) -> str | None:
    """Return org_id for the given agent token, or None."""
    if not token:
        return None
    row = await conn.fetchrow(
        "SELECT org_id FROM agents WHERE token = $1", token
    )
...
async def woocommerce_webhook(request):
    ...
    async with pool.acquire() as conn:
        # Resolve org from the token in the URL (?token=...)
        token = request.args.get("token", "")
        org_id = await _resolve_token(token, conn)
        if not org_id:
        ...
        existing = await conn.fetchrow(
            "SELECT id, status FROM woo_orders WHERE woo_order_id=$1 AND org_id=$2",
            woo_order_id, org_id,
        )
        if existing:
            ...
            order_uuid = await conn.fetchval(
                "INSERT INTO woo_orders (woo_order_id, raw_json, status, org_id) VALUES ($1, $2::jsonb, 'pending', $3) RETURNING id",
                woo_order_id, json_module.dumps(payload), org_id,
            )

        # Parse line items and match to this org's variants
        ...
        for item in line_items:
            ...
            variant = await conn.fetchrow(
                """SELECT pv.id FROM product_variants pv
                   ...
                   WHERE pv.sku=$1 AND pv.is_active=true AND p.org_id=$2 LIMIT 1""",
                sku, org_id,
            )
            if variant:
                await conn.execute(
                    """INSERT INTO print_jobs (type, woo_order_id, variant_id, label_type, quantity, status, org_id)
                    VALUES ('woocommerce', $1, $2, 1, $3, 'queued', $4)""",
                    order_uuid, variant["id"], quantity, org_id,
                )
            else:
